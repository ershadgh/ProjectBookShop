...
namespace ProjectBookShop.Areas.Api.Classes
{
    public class ApiResult
    {
        ...
        //اگر مقدار پراپرتی خالی بود اصلا در خروجی جیسون این پراپرتی نمایش نمی دهد
    ...
    public class ApiResult<TData> : ApiResult where TData : class
    {
        ...
        public static implicit operator ApiResult<TData>(BadRequestObjectResult result)
        {
            ...
            {
                var errorsMessages = identityErrors.Select(p => p.Description).Distinct();
                message.AddRange(errorsMessages);
            }
            else
            {
                message.Add(result.Value.ToString());
